...
public abstract class AbstractHandworkScreen<T extends AbstractHandworkContainer> extends ContainerScreen<T> {
	...
	public AbstractHandworkScreen(T screenContainer, PlayerInventory inv, ITextComponent titleIn) {
	...
	protected boolean itemStackMoved(int keyCode, int scanCode) {
		if (this.hoveredSlot == this.container.getSlot(((AbstractHandworkContainer) container).getHandSlot())) {
			return false;
		}else {
