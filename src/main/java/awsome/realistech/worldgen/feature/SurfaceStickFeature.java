...
public class SurfaceStickFeature extends Feature<NoFeatureConfig> {
	
	public SurfaceStickFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> configFactoryIn) {
	...
	public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand,
		...
		if (worldIn.getWorld().getWorldType() != WorldType.FLAT)
        {
            ...
            for (int i = 0; i < sampleLimit; i++)
            {
                ...
                if (worldIn.getBlockState(samplePos) != Registration.STICK.get().getDefaultState())
                {
                    ...
                    {
                    	worldIn.setBlockState(samplePos, Registration.STICK.get().getDefaultState(), 2 | 16);
                    	placed = true;
                    }
                }
            }
